...
We have obtained UK road safety data from stats19 . In this dataset, vehicles and their occupants are identified using a detailed vehicle type classification. We have created a simplified form of this classification, to more clearly and simply represent the types of vehicles involved in collisions with cyclists or pedestrians. This reclassification system is codified as a function in our new `traffiCalmr` R package.

The assignment of vehicles and casualties to simplified vehicle types and the numbers in each category are shown in Table 1 below.

```{r table1, message = FALSE}
readr::read_csv() %>%
  kableExtra::kable(caption = "Original and simplified vehicle type categories and the number of vehicles and casualties assigned to each category, over the period 2009-2018", booktabs = T)
```
