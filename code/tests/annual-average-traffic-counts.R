...
# county aggregation
traffic_sf_las = traffic_cyclable %>%
...
nrow(traffic_sf_las)
# [1] 208
mapview::mapview(traffic_sf_las)
traffic_sf_las_joined = sf::st_join(
  traffic_sf_las,
  counties_uas_gb %>% select(name = ctyua19nm)
)
raw_la_county_lookup = traffic_sf_las_joined %>%
...
raw_la_county_lookup_point = traffic_sf_las_joined %>%
...
# where is it?
missing_count_point = traffic_sf %>%
  filter(count_point_id == 50974)
missing_count_point2 = traffic_sf %>%
  filter(count_point_id == 10820)
missing_count_point3 = traffic_sf %>%
  filter(count_point_id == 50728)
missing_count_point4 = traffic_sf %>%
  filter(count_point_id == 82090)
missing_count_point5 = traffic_sf %>%
