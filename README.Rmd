...
accidents = get_stats19(1990:2019, "ac", ask = FALSE)
casualties = get_stats19(1990:2019, "cas", ask = FALSE)
cas = left_join(accidents, casualties)
saveRDS(cas, "cas_1990.Rds") # 31 MB
...
saveRDS(cas_long, "cas_long_1990.Rds")
piggyback::pb_upload("cas_long_1990.Rds")
```
...
cas_long = readRDS("cas_long_1990.Rds")
# ggplot(cas_long) +
...
cas_long = cas_long %>% 
  filter(Mode != "Total") %>% 
  mutate(Number = Number * 12)
p1 = ggplot(cas_long) +
  geom_line(aes(month, Number, colour = Severity)) +
  geom_smooth(aes(month, Number, colour = Severity), fill = NA) +
  facet_grid(. ~ Mode) +
  scale_y_log10(breaks = c(50, 100, 250, 500, 1000, 2500, 5000, 10000, 25000, 50000)) +
  ylab("Casualties/year") +
  xlab("") +
  geom_vline(xintercept = as.Date("2010-01-01"))
p1
saveRDS(p1, "p1_ggplot_1979_2019_casualties_per_month.Rds")
